...
#define START_TIME 3000    // us TIME for I2C Ready 2500 pour decoder 3000 pour switch SDA low 1us a 1,8ms ou 2,4ms 
...
int cpulse()
{
  digitalWrite(PDEBUG,LOW) ;
  usleep(1);
  digitalWrite(PDEBUG,HIGH) ;
  return 0;
}
...
cpulse() ;
MUTEX_UNLOCK(Pmutex[M_SYSTEM+2]);
if (i2c_fdebug) gPrintf(SCR,"fail to talk to slave adr=%u .\n",adr);
return -2;
}
...
cpulse() ;
MUTEX_UNLOCK(Pmutex[M_SYSTEM+2]);
if (i2c_fdebug) gPrintf(SCR,"Failed to write to the i2c bus (req %d read %d adr=%u)\n",len,err,adr);
//close (file_i2c);
return -1 ;
}
...
cpulse()   ;
MUTEX_UNLOCK(Pmutex[M_SYSTEM+2]);
if (i2c_fdebug) gPrintf(SCR,"Failed to talk to slave adr=%u\n",adr);
return -2;
}
...
cpulse() ;
MUTEX_UNLOCK(Pmutex[M_SYSTEM+2]);
if (i2c_fdebug) gPrintf(SCR,"Failed to read from the i2c bus req %d read %d adr=%u",len,err,adr);
return -1;
}
...
Tx_Retry:
  ...
  {  pbuf = buf1  ;
	 ...
	 { if (i2c_fdebug)  gPrintf(SCR,"Too much Tx-Retry1 %d adr=%u\n",retry1_ct,i2c_adr); 
           return -1;
         }
	 if (i2c_fdebug) gPrintf(SCR,"Tx-Retry1 %d adr=\n",retry1_ct,i2c_adr);
  }
  ...
  { buf2[0]= (char)0x5A;
	...
	{ if (i2c_fdebug) gPrintf(SCR,"Too much Tx-Retry2 %d adr=%u\n",retry2_ct,i2c_adr); 
          return -1;
        }
	if (i2c_fdebug) gPrintf(SCR,"Tx-Retry2 %d adr=%u\n",retry2_ct,i2c_adr);
  }
  ...
  { sprintf(tbuf,"TxI2C(%u)",i2c_adr);
    //myPrintf("TxI2C",(uint8_t *)pbuf,nb);
    myPrintf(tbuf,(uint8_t *)pbuf,nb);
  }
  ...
  if (err < 0) 
  { // if (err == -2) return -1;
	if (i2c_fdebug) {gPrintf(SCR,"Tx Error, Retry=%d adr=%u\n",ct,i2c_adr);}
    if (ct > MAX_RETRY_HW)
		return -1;
	ct++;
    usleep(1000);
...
if (i2c_fdebug) 
{ sprintf(tbuf,"RxI2C(%u)",i2c_adr);
//myPrintf("RxI2C",(uint8_t *)bufi2c,i2c_ptrx2);
myPrintf(tbuf,(uint8_t *)bufi2c,i2c_ptrx2);

}

if ( (bufi2c[i2c_ptrx2-2] != (char)(0xFF & mycrc) ) || (bufi2c[i2c_ptrx2-1] != (char)(0xFF & (mycrc>>8))) )
{ if (i2c_fdebug) gPrintf(SCR,"Error CRC adr=%u!\n",i2c_adr);
  error_crc++;
  //////////////////////////
  if (f_tx && (state == 0) )
  /////////////////////////
...
{ if (i2c_fdebug) gPrintf(SCR,"Error NAD %d adr=%u !\n",bufi2c[0],i2c_adr);
  error_pcb++;
  return -1;
}
...
{error_pcb++;
 if (i2c_fdebug) gPrintf(SCR,"Error PCB %2.2X adr=%u\n",0xFF & bufi2c[1],i2c_adr);
 /////////////////////////////////////////////////////////////
...
{ int dadr=0 ;
  ...
  { 
   //MUTEX_LOCK(Pmutex[M_SYSTEM+3]);
    ...
    digitalWrite(i2c_E_pins[pin_E],LOW);
    usleep(RESET_TIME);
    //digitalWrite(i2c_decoder_pins[0],HIGH);
    digitalWrite(i2c_E_pins[pin_E],HIGH);
...
{ int err,i;
 ...
 { if (i2c_fdebug)
   ...
   { for(i=0;i<6;i++)
     ...
     for(i=0;i<nbdecoder;i++)
	 { if (i2c_fdebug) gPrintf(SCR,"Pin_E %d => output\n",i2c_E_pins[i]);
       pinMode(i2c_E_pins[i], OUTPUT) ;
       digitalWrite (i2c_E_pins[i], HIGH) ;
	 }
